...
class Datasets(Dataset):
    def __init__(self, input_dir, filetype, file_handler = None):

        # Take input from user
        self.input_dir = '{0}/{1}'.format(input_dir,filetype)
        self.filetype = filetype
        self.data_files = os.listdir(self.input_dir)

        #Create logger
        self.dlog = get_logger('Datasets', file_handler)
        self.dlog.info('List of data files: {0}'.format(self.data_files))
